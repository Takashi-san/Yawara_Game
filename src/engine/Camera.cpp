...
void Camera::Update(float dt) {
	...
	if ((focusA != nullptr) && (focusB != nullptr)) {
		...
		pos.x = pos.x + focusA->box.Center().x - Game::GetInstance().GetWindowSize().x/2 /* + Game::GetInstance().GetWindowSize().x/2*/;
		pos.y = pos.y + focusA->box.Center().y - Game::GetInstance().GetWindowSize().y/2 /*+ Game::GetInstance().GetWindowSize().y/2*/;
	} else if (focusA != nullptr) {
		pos.x = focusA->box.Center().x - Game::GetInstance().GetWindowSize().x/2 /*+ Game::GetInstance().GetWindowSize().x/2*/;
		pos.y = focusA->box.Center().y - Game::GetInstance().GetWindowSize().y/2 /*+ Game::GetInstance().GetWindowSize().y/2*/;
	} else {
